...
public class C3698FilteringSearchForInvoiceNumber extends MarketplaceTestTemplate {

  @Test(groups = {"uat", "dev"})
  @TestRailCase(projectId = TEST_RAIL_PROJECT_ID, caseId = "C3698")
  public void filteringSearchForInvoiceNumberTest() throws Exception {
    EhurtAppUser marketUser = getEhurtAppUser(EhurtAppUserRole.MARKETPLACE);
    val ean = Product.POLISH_SAUSAGES.getEAN();//"7613036908290";
    val mName = Product.POLISH_SAUSAGES.getMarketName();//"SOS WINIARY AZJATYCKI 250ML";
    ...
    FilterForm filterForm = invoicePage.clickFilterBtn();
    filterForm.clickInvoiceNumberBtn();
    filterForm.typeInvoiceNumberValue(invoiceNumber);
    filterForm.clickLoupeBtn();
    invoicePage.clickHideResults();
    invoicePage.clickSortUp();
    invoicePage.checkInvoiceNumber(invoiceNumber, errors);
